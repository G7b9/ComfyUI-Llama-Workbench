...
All five `.json` files in this directory are ComfyUI workflow exports. Import
...
| `05_qwen-image-2.1-pe-t2i-gguf.json` | End-to-end PE-T2I GGUF → native Qwen-Image-2.1 generation → saved image | Set the PE GGUF, `llama-server`, and local matching `system_prompt.txt` paths. Install the diffusion model, text encoder, and VAE selected in the loader nodes. PE Resolution uses the model ratio by default or a selected manual override. The PE server auto-unloads before sampling. The workflow includes no model or official prompt asset. |
